# Controller application programming interface (API) <!-- omit from toc -->
...
Create a _crosswordController_ by passing the `model` and the `gridParent` and `cluesParent` DOM elements into the constructor:
...
```js
// events is an array of event names, such as ["click","hover"].
// A single event must still be passed in an array.
// handler is your event handler function
